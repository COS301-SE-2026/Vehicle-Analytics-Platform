...
describe('Fleet Analytics Controller', () => {
  ...
  describe('GET /api/fleet/analytics', () => {
    ...
    test('should coerce string-typed numeric fields from the DB into real numbers', async () => {
      mockQuery.mockImplementation((sql) => {
        ...
        if (queryStr.includes('driver_daily_safety_scores') && queryStr.includes('group by score_date')) {
          return Promise.resolve({
            rows: [{ score_date: '2026-07-20', avg_score: '85.5', vehicle_count: '10' }],
            rowCount: 1,
          });
        }
        if (queryStr.includes('driver_daily_safety_scores') && queryStr.includes('group by vehicle_id')) {
          return Promise.resolve({
            rows: [
              {
                vehicle_id: 'V001',
        ...
        if (queryStr.includes('event_type') || queryStr.includes('event_breakdown')) {
          return Promise.resolve({
            rows: [{ type: 'harsh_braking', count: '5' }],
            rowCount: 1,
          });
        }
        if (queryStr.includes('vehicle_contributions') || queryStr.includes('total_events')) {
          return Promise.resolve({
            rows: [{ vehicle_id: 'V001', total_events: '12' }],
            rowCount: 1,
          });
        }
    ...
    test('should handle invalid period (defaults or gracefully handles)', async () => {
      const response = await request(app)
  ...
  describe('GET /api/fleet/vehicle/:vehicleId/scores', () => {
    ...
    test('should handle non-existent vehicle with empty scores array', async () => {
