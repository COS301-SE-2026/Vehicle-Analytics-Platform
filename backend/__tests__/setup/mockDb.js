...
function setupMockData() {
  ...
  mockQuery.mockImplementation((sql, params) => {
    ...
    if(sql.includes('COUNT(*) as total_vehicles')){


      
      return Promise.resolve({ rows: [{total_vehicles: 15, active_vehicles: 12}] });
    ...
    if(sql.includes('vehicle_events_hourly')){


      
      return Promise.resolve({rows: [{ alerts_today: 15 }]});
    ...
    if(sql.includes('vehicle_daily_distance')){


      
      return Promise.resolve({rows: [{ distance_today: 450.5 }]});
    ...
    if(sql.includes('SELECT COUNT(*) FROM vehicles')){
    ...
    if(sql.includes('INSERT INTO geofences')){
      ...
      return Promise.resolve({rows: [{ id: 1, name: 'Test Zone', vehicle_id: null, trigger_type: 'both', is_active: true }]});
    ...
    if(sql.includes('SELECT * FROM trips')){
      ...
      return Promise.resolve({
        ...
        rows: [
          ...
          {trip_id: 1, vehicle_id: 'V001', start_time: new Date(), end_time: new Date(), distance_km: 45.5, avg_speed_kmh: 45.5, max_speed_kmh: 85.0, safety_score: 82, status: 
'completed'},
    ...
    if(sql.includes('SELECT * FROM cluster_points')){
      ...
      return Promise.resolve({
        ...
        rows: [
          ...
          {vehicle_id: 'V001', cluster_id: 'cluster1', center_lat: -29.85, center_lng: 31.0, point_count: 25, first_seen: new Date(), last_seen: new Date()},
        ],
...
'Good'},
    ...
    if(sql.includes('SELECT * FROM driver_daily_safety_scores WHERE score_date = $1')){
      ...
      return Promise.resolve({
        ...
        rows: [
          ...
          {vehicle_id: 'V001', score_date: '2025-01-01', safety_score: 85, harsh_brakes: 2, harsh_accelerations: 1, harsh_cornering: 0, crashes: 0, total_events: 3, classification: 
'Good'},
