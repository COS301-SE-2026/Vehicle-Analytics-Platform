...
// Helper to validate and parse integer IDs
const isValidId = (id) => {
...
function validateTopLevelFields({ name, fleet_group_id }) {
  ...
  if (!isValidId(fleet_group_id)) {
    errors.push('fleet_group_id must be a valid positive integer');
  }
...
function validateSpeedThreshold(condition_params) {
  ...
  } else if (typeof max_speed_kmh !== 'number' || max_speed_kmh <= 0 || max_speed_kmh > 300) {
    errors.push('max_speed_kmh must be a positive number up to 300');
  }
...
function validateRuleFields({ name, fleet_group_id, condition_type, condition_params }) {
  ...
  // Security Fix: Prevent type-coercion bugs (in JS, typeof null === 'object')
  if (!condition_params || typeof condition_params !== 'object' || Array.isArray(condition_params)) {
    errors.push('condition_params is required and must be an object');
    return errors;
  }
...
async function createRule(req, res) {
  const managerId = isValidId(req.user?.id);
  const { name, fleet_group_id, condition_type, condition_params } = req.body;

  if (!managerId) {
    return error(res, 'Unauthorized user context', 401);
  }
  ...
  try {
    ...
    // Security Fix: Hide raw database message from client
    console.error('Create custom alert rule error:', err);
    return error(res, 'An internal error occurred while creating the rule', 500);
  }
}
...
async function listRules(req, res) {
  ...
  try {
    ...
    return error(res, 'An internal error occurred while fetching rules', 500);
  }
}

async function getRule(req, res) {
  const managerId = isValidId(req.user?.id);
  const ruleId = isValidId(req.params.id);
  ...
  try {
    ...
    return error(res, 'An internal error occurred while fetching the rule', 500);
  }
}
  ...
  try {
    ...
    const result = await pool.query(
      `UPDATE custom_alert_rules
       ...
       RETURNING *`,
      [fleet_group_id, name, condition_type, condition_params, status, ruleId, managerId]
    );
    ...
    return error(res, 'An internal error occurred while updating the rule', 500);
  }
}
...
async function setRuleStatus(req, res) {
  ...
  try {
    ...
    return error(res, 'An internal error occurred while updating the rule status', 500);
  }
}
...
async function deleteRule(req, res) {
  ...
  try {
    ...
    return success(res, { id: ruleId }, 200);
  } catch (err) {
    console.error('Delete custom alert rule error:', err);
    return error(res, 'An internal error occurred while deleting the rule', 500);
  }
}
