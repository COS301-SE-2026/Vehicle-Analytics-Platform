...
async function generateReport(req, res) {
  ...
  try {
    ...
    // Parallelize execution of current period, previous baseline period, and trend analytics
    const [current, previous, trends] = await Promise.all([
      runAnalytics(pool, scope.vehicleIds, period),
      period.previous
        ? runAnalytics(pool, scope.vehicleIds, period.previous)
        : Promise.resolve(null),
      buildWeeklyTrends(pool, scope.vehicleIds, period),
    ]);
