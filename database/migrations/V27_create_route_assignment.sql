...
-- network (V28), and the resulting LINESTRING is stored as the EXPECTED
-- CORRIDOR for that vehicle. Live telemetry is then checked against it.
--
-- The route is a reference geometry, not directions -- nobody is
-- instructed to drive it. That's why one-way and turn restrictions are
-- ignored upstream: the only requirement is that the line hugs real roads
-- closely enough that a bus following a curve isn't flagged as deviating.
...
--
-- A plain UNIQUE(vehicle_id) would have made route_name and `active`
-- pointless: you could never keep "Morning Delivery" alongside
-- "Afternoon Loop", nor retain a superseded route as history. A partial
-- unique index gives the real guarantee (no ambiguity about which route a
-- vehicle is being monitored against) without foreclosing either.
...
-- Editing a route means reordering/moving these and regenerating -- you
-- can't recover intent from the LINESTRING alone.
...
-- One row per deviation INCIDENT, not per off-route ping.
--
-- Without the state machine below, a bus 200m off-route for ten minutes at
-- ~7s pings would write ~85 rows for a single incident -- the same failure
-- mode geofence_state solves for entry/exit and security_marker_dedup_window
-- solves for tow alerts. resolved_at is what makes this an incident record:
-- NULL means still off-route.
...
-- Current on/off-route state per vehicle. Directly analogous to
-- geofence_state: the trigger writes an event only on a TRANSITION, so
-- this is what it compares against.
--
-- last_checked_time is the telemetry timestamp, not NOW() -- it doubles as
-- the out-of-order guard, so a late-arriving old ping can't flip state
-- backwards and fabricate a deviation.
...
--
-- Dashboard, map, vehicle popup and alerts all read THIS, rather than each
-- re-deriving the same answer by joining routes + route_state +
-- route_deviation_events. Three consumers writing that join three ways is
-- how they end up disagreeing about whether a vehicle is off-route.
--
-- Returns EVERY vehicle, not just routed ones: a vehicle with no route
-- reads NO_ROUTE rather than vanishing from the result, so the frontend
-- never has to distinguish "no route assigned" from "query returned
-- nothing". Same reasoning as get_current_safety_scores_all_vehicles.
--
-- A view rather than a function so it composes: callers can filter it,
-- join it, or select single columns and let the planner push predicates
-- down, none of which works cleanly through a set-returning function.
-- ---------------------------------------------------------------------------
