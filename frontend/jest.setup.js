...
jest.mock('radix-ui', () => {
  ...
  const el = (tag, dataSlot) => {
    const Component = ({ children, className, 'data-slot': __ds, ...props }) =>
  ...
  const img  = (slot) => {
    const Component = ({ 'data-slot': __ds, className, ...props }) =>
  ...
  return {
    ...
    Separator: {
      Root: ({ 'data-slot': __ds, className, orientation, decorative, ...props }) =>
