...
describe('CreateAlertRuleModal', () => {
  ...
  test('renders nothing when isOpen is false', () => {
    ...
    const { container } = render(
      <CreateAlertRuleModal isOpen={false} onClose={onClose} onCreated={onCreated} fleetGroups={fleetGroups} />
    );
  ...
  test('successfully creates a rule', async () => {
    ...
    axios.post.mockResolvedValue({ data: { data: { id: 'rule-100', name: 'Highway Speeding' } } });
    ...
    expect(mockToast.success).toHaveBeenCalledWith('Alert Rule Created Successfully.', 'Your new rule is now active.');
  });

  test('shows error + toast on API failure', async () => {

    axios.post.mockRejectedValue({ response: { data: { message: 'Name already exists' } } });
    ...
    expect(mockToast.error).toHaveBeenCalledWith('Failed to Create Rule.', 'Please check your connection and try again.');
