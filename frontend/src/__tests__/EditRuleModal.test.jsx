...
describe('EditAlertRuleModal', () => {
  ...
  test('renders nothing when isOpen is false', () => {
    const { container } = render(
      <EditAlertRuleModal isOpen={false} onClose={onClose} onUpdated={onUpdated} rule={baseRule} fleetGroups={fleetGroups} />
    );
    expect(container).toBeEmptyDOMElement();
  });

  test('seeds the form from the rule prop', () => {
    render(<EditAlertRuleModal isOpen={true} onClose={onClose} onUpdated={onUpdated} rule={baseRule} fleetGroups={fleetGroups} />);
    ...
    render(
      <EditAlertRuleModal
        ...
        rule={{
          ...
          condition_params: { start_time: '22:00', end_time: '06:00', restricted_days: ['Mon'] },
        }}
        fleetGroups={fleetGroups}
      />
    );
  ...
  test('successfully updates a rule', async () => {
    axios.put.mockResolvedValue({ data: { data: { id: 'rule-99', name: 'Updated Name' } } });
    ...
    expect(mockToast.success).toHaveBeenCalledWith('Alert Rule Updated Successfully.', 'Changes to "Updated Name" have been saved.');
  });

  test('submits status: "inactive" after switching it via the button', async () => {
    axios.put.mockResolvedValue({ data: { data: { id: 'rule-99', name: 'Highway Speeding' } } });
  ...
  test('shows error + toast on API failure', async () => {
    axios.put.mockRejectedValue({ response: { data: { message: 'Name already exists' } } });
    ...
    expect(mockToast.error).toHaveBeenCalledWith('Failed to Update Rule.', 'Please check your connection and try again.');
