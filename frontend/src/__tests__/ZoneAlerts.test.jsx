...
// Own concerns (drawer open/close, its own data fetching presumably) are
// out of scope here -- stubbed to just expose the open flag.
// NOTE: jest.mock() resolves its path from THIS file's location, not from
// ZoneAlerts.jsx's. ZoneAlerts.jsx imports "./ZoneActivityDrawer" relative
// to itself (src/components/geofence/); the equivalent path from here
// (src/__tests__/) is "../components/geofence/ZoneActivityDrawer". Mocking
// "./ZoneActivityDrawer" from this file would target a different, unrelated
// module id and silently fail to intercept the real import.
jest.mock("../components/geofence/ZoneActivityDrawer", () => ({
  ZoneActivityDrawer: ({ open }) => (
    <div data-testid="activity-drawer" data-open={open} />
  ),
}));

// Pagination (rendered for real below, not mocked) imports ChevronLeft and
// ChevronRight from lucide-react -- those must be included here too, or
// Pagination throws trying to render an undefined component the moment any
// alerts exist.
...
describe("ZoneAlerts: rendering alerts", () => {
  ...
  it.each([
    ...
    ["other", "icon-other"],
    ["unrecognised_kind", "icon-other"], // falls back to Bell
...
describe("ZoneAlerts: clear flow", () => {
  ...
  it("re-enables the Clear button and logs if the delete call fails", async () => {
    ...
    // Alerts were NOT cleared client-side on failure -- still visible.
