    ...
    return (
        <div>
            ...
            <div className="flex flex-wrap gap-2">
                {stats.map((stat) => (
                    <div key={stat.label} data-testid={`stat-${stat.label}`} className="flex items-center gap-1.5 border border-fleet-border rounded-lg px-3 py-1.5">
                    <span data-testid={`stat-count-${stat.label}`} className={`text-sm font-bold ${stat.count > 0 ? 'text-fleet-alert' : 'text-fleet-text'}`}>
                        {stat.count}
                    </span>
